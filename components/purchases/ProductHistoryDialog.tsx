...
function getProductSaleLines(sales: Sale[], productName: string, year: number) {
  return sales.flatMap((sale) =>
    (sale.items ?? [])
      .filter((item) => item.productName === productName)
      .map((item, index) => ({
        lineId: `${sale.id}-${index}`,
      ...
      .filter((line) => getNepaliYear(line.saleDate) === year),
  );
}
...
export default function ProductHistoryDialog({
  ...
  const productSales: ProductSaleLine[] = getProductSaleLines(
    sales,
    product.name,
    currentYear,
  );
  ...
  const totalSalesValue = productSales.reduce(
    (sum, line) => sum + line.quantitySold * line.salePrice,
    0,
  );
              ...
              <Table>
                ...
                <TableBody>
                  {productSales.length > 0 ? (
                    productSales.map((line) => (
                      <TableRow
                        key={line.lineId}
                        className="hover:bg-gray-100 dark:hover:bg-gray-700/50"
                      >
                        <TableCell className="text-gray-700 dark:text-gray-300">
                          {formatNepaliDateForTable(line.saleDate)}
                        </TableCell>
                        <TableCell className="font-medium text-gray-900 dark:text-gray-100">
                          {line.client}
                        </TableCell>
                        <TableCell className="text-gray-700 dark:text-gray-300">
                          {line.quantitySold} units
                        </TableCell>
                        <TableCell className="text-gray-700 dark:text-gray-300">
                          Rs {line.salePrice.toLocaleString()}
                        </TableCell>
                        <TableCell className="font-semibold text-green-600 dark:text-green-400">
                          Rs{" "}
                          {(
                            line.quantitySold * line.salePrice
                          ).toLocaleString()}
                        </TableCell>
                      </TableRow>
                    ))
